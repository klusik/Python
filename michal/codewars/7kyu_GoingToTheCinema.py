...
# kod funguje, ale bezi dlouho -> sample funguje, attempt vraci timeout -> reseni neuploadovano
# v math modulu existuje funkce ceil(), ktera vraci, nejnizsi cele cislo vetsi nez X ...
def movie(card, ticket, perc):
    ...
    # iterations
    i = 0
    planBprice.append(card)
    ticketB = ticket
    while True:
        if sum(planAprice) > sum(planBprice):
            break
        else:
            planAprice.append(ticket)

            ticketB = ticketB * perc
            # print('adding reducing ticket value: ' + str(ticketB))
            planBprice.append(round(ticketB))

            i = i+1

            # print('sum price of plan A: ' + str(sum(planAprice)))
            # print('sum price of Plan B: ' + str(sum(planBprice)))
